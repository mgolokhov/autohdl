...
from autohdl import VERILOG_VHDL_FILE_EXT
from autohdl import ALDEC_PATH
import logging
...
def updateDeps(files):
    files = [os.path.abspath(ALDEC_PATH + f[7:-1]) for f in files]
    ...
    files = [i for i in files if os.path.splitext(i)[1] in VERILOG_VHDL_FILE_EXT]
    # modify dependencies in kungfu.py
    #build.updateDeps(files)


def synchBuild():
    # relative to dsn/script dsn/aldec/config.cfg
    config = os.path.abspath(ALDEC_PATH + '/compile.cfg')
    files = set()
    shaOld = None
    while True:
        try:
...
subprocess.call([aldec, ALDEC_PATH + '/wsp.aws'])
moveInRepo()
