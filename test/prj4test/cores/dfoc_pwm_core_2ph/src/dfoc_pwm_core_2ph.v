...
//Модуль расчет напряжений на фазах двухфазного	синхронного двигателя по методу Field Oriented Control (FOC)
//
//
//Входы:
...
module dfoc_pwm_core_2ph(
    ...
    always @(posedge rst, posedge clk)		 //Селектор переключающий входы умножителя mult19
        if(rst)
            ...
            reg [18:0]a19p;	 //19 битный вход умножителя mult19
    reg [18:0]b19p;	 //19 битный вход умножителя mult19
    
    always @(*)				//Переключение входов умножителя
        begin
            a19 <= {cp_a[15],cp_a[15],cp_a[15],cp_a};
            b19 <= {cp_b[15],cp_b[15],cp_b[15],cp_b};
            a19p <= {p_a[15],p_a[15],p_a[15],p_a};
            b19p <= {p_b[15],p_b[15],p_b[15],p_b};
        end

    
    wire [36:0]c19p;  //37 битный выход умножителя mult19
    wire [15:0]cp;
    assign cp = c19p[30:15];
    
    mult19 m19_dfoc3 (	 //19 битный знаковый умножитель mult19
        .a(a19p),
        .b(b19p),
        .c(c19p)
        );	
 ...
 assign c32 = -50*phi[31:16];
    ...
    assign sincos_start = start;// p_rdy;//start;
